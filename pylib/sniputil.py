...
def snip(trigger, desc, body, flags="", aliases=[], trimBody=True,
    ...
    if spacesToTabs:
        def spaceRepl(m):
            numTabs  = len(m.group(1)) / spacesToTabs
            return '\t' * numTabs
        tabRe = re.compile(r'^((?: {%d})+)' % (spacesToTabs,), re.M)
        body = tabRe.sub(spaceRepl, body)
...
def bsnip(trigger, desc, body, flags="", aliases=[], trimBody=True,
    ...
    snip(trigger, desc, body, flags="b" + flags.replace("b", ""),
            aliases=aliases, trimBody=trimBody, spacesToTabs=spacesToTabs)
